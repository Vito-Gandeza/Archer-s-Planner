...
test("hourRange makes room for class meetings, not just deadlines", () => {
  assert.deepEqual(hourRange([], [8, 24], [meet(0, "09:15:00", "10:45:00")]), [8, 12]);
  // A class ending exactly on the hour must not pull in an extra empty hour.
  assert.deepEqual(hourRange([], [8, 24], [meet(0, "11:00:00", "14:00:00")]), [10, 15]);
});
