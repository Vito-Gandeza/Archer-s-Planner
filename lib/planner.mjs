...
/**
 * Hour rail bounds for the week grid.
 *
 * Always covers the teaching day (DAY_WINDOW) even when the only deadlines are
 * at 11:59pm — otherwise the grid collapses to a four-hour stripe at the bottom
 * of the screen and reads as broken rather than empty. Content outside that
 * window expands it; nothing ever shrinks it below.
 */
export const DAY_WINDOW = [8, 18];

export function hourRange(deadlines, fallback = DAY_WINDOW, meetings = []) {
  ...
  let lo = Math.min(fallback[0], ...(hours.length ? [Math.max(0, Math.min(...hours) - 1)] : [fallback[0]]));
  let hi = Math.max(fallback[1], ...(hours.length ? [Math.min(24, Math.max(...hours) + 2)] : [fallback[1]]));
  lo = Math.max(0, lo);
  hi = Math.min(24, hi);
  while (hi - lo < 4) {
    if (hi < 24) hi++;
...
/**
 * Canvas hands back a course code with the section glued on: DLSU sends
 * "MICPROS_E25", "NSTP101 - EA4", "LIBRES-General AY 2026-2027". The part
 * before the first separator is the code a student actually says out loud, and
 * the one printed on a schedule — which is what image matching has to hit.
 */
export function shortCode(raw, fallback = "") {
  const first = String(raw ?? "")
    .trim()
    .split(/[\s_\-–—/]+/)[0];
  return (first || String(fallback ?? "").trim() || "").slice(0, 32);
}

/**
 * Something months past due is abandoned, not urgent. Without this a survey
 * nobody ever intended to finish sits in the "next up" slot forever, pushing
 * the actual work down the page.
 */
export const STALE_OVERDUE_DAYS = 30;

export function isStale(d, now = new Date(), days = STALE_OVERDUE_DAYS) {
  if (!d.due_at) return false;
  return new Date(d.due_at).getTime() < now.getTime() - days * DAY_MS;
}

/**
 * Splits open deadlines into what deserves attention now and what has been
 * hanging around so long it should be out of the way but not forgotten.
 */
export function triage(deadlines, now = new Date()) {
  const open = deadlines.filter((d) => d.status === "open");
  const dated = open.filter((d) => d.due_at);
  const stale = dated.filter((d) => isStale(d, now));
  const fresh = dated.filter((d) => !isStale(d, now));
  return {
    stale: stale.sort((a, b) => b.due_at.localeCompare(a.due_at)),
    overdue: fresh.filter((d) => deadlineState(d, now) === "overdue").sort((a, b) => b.due_at.localeCompare(a.due_at)),
    undated: open.filter((d) => !d.due_at),
  };
}
